...
$(document).ready(function () {
    ...
    if (typeof jQuery === 'undefined') {
        console.warn('lmSelecionador2: JavaScript precisa do jQuery');
        return;
    }
    //if (typeof JsRender === 'undefined') {
    //    console.warn('lmSelecionador2 JavaScript precisa do JsRender');
    //    return;
    //}
    ...
    // Forma de Usar
    //----------------------
    //$("#meu-componente-input-text").lmSelecionador2(
    //    {
    ...
    $.fn.lmSelecionador2 = function (fn, options) {

        var namefunction = "lmSelecionador2";

        var originalElement = this;

        // Verifica se o elemento selecionado existe e é um input
        if (this.length == 0) { throw new Error('lmSelecionador2: Selector inválido ' + this.selector + ' - Por favor, verifique se o ID ou Classe CSS nome utilizado está correto.') };
        if (!this.is('input')) { throw new Error('lmSelecionador2: Elemento deve ser um "Input"') };
        ...
        if (!$.lmSelecionador2scopes)
            $.lmSelecionador2scopes = {};
        var scope = $.lmSelecionador2scopes[originalElement.attr("id")];
        if (!scope) {
            scope = {};
            $.lmSelecionador2scopes[originalElement.attr("id")] = scope;
            scope.originalElement = originalElement;
        }
    };

    $.fn.lmSelecionador2Old = function (fn, options) {
        ...
        switch (('' + fn).toLowerCase()) {
            ...
            case "readonly":
                ...
                break;
            default:
                throw new Error('lmSelecionador2: ' + originalElement.attr('id') + ' "' + fn + '" não é uma função válida');
        }
        ...
        function mountScope(scope, options) {
            ...
            if (settings.habilitarDetalhe)
                settings.columns.push({
                    text: 'Detalhe', datafield: 'Detalhe', columntype: 'button', cellsrenderer: function () {
                        return "Edit";
            ...
            mymodal.find("#listBox").jqxGrid(
                {
                    source: [],
                    columns: settings.columns, // TODO: Adicionar opção de adicionar botão de detalhes
                    sortable: true,
                    width: '100%',
                    height: '240',
            ...
            scope.clear = function () {
                ...
                $(scope.settings.datafields).each(function () {
                    ...
                    if (datainput.length == 0) { throw new Error('lmSelecionador2.datafields: Selector inválido "' + this.datainput + '"- Por favor, verifique se o ID está correto.') };
